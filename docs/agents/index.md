...
**Connect** page in the web UI makes wiring one up a single click: cairn detects the agents
installed on your machine and writes each one's MCP config for you.

## How auto-connect works

The cairn process already runs locally with your permissions, so it writes the config files
itself — the same code path whether you're in the desktop app or `cairn web` in a browser. For
each agent, Connect:
...
A **Disconnect** (the unplug button on a connected card) removes only cairn's entry, leaving
...
safe to commit and share. Windsurf and Antigravity have no project-level config, so they're
